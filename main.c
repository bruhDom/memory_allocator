...
int main() {
    ...
    printf("\nmemory block 1 free status: %d", headerBlock1->is_free);

    if(headerBlock1->is_free) 
        printf("\nmemory has been freed\n");

    //---- ASK FOR SOME MEMORY FROM FREED AREA BACK ----

    void* memory4 = mem_alloc(page_size);

    printf("\nmemory4 starts at: %p\n", memory4);

    void *header4 = memory4 - sizeof(mem_block);
    mem_block *headerBlock4 = (mem_block*)header1;

    memset(memory4, 0xDD, headerBlock4->size - sizeof(mem_block));
    printf("\nmemory4 contains: 0x%02X\n", ((uint8_t*)memory4)[0]);
